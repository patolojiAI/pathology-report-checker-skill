...
class MultiPassAnalyzer:
    ...
    def __init__(self, provider: str, model: str, base_url: str = "",
        ...
        self.model = model
        self.extraction_model = extraction_model or model  # Bigger model for Pass 1
    ...
    def _call(self, prompt: str, system: str, pass_name: str,
              model_override: str = None) -> str:
        model = model_override or self.model
        return call_llm(prompt, self.provider, model,
                        self.base_url, system, pass_name)

    def _call_json(self, prompt: str, system: str, pass_name: str,
                   model_override: str = None) -> dict:
        raw = self._call(prompt, system, pass_name, model_override)
        result = extract_json(raw)
        if "_error" in result:
            self.log(f"  ↳ JSON retry...")
            raw = self._call(
                prompt + "\n\nRETURN ONLY VALID JSON. Nothing else.",
                system, pass_name, model_override
            )
            result = extract_json(raw)
        return result

    def pass1_extract(self, report_text: str) -> dict:
        if self.extraction_model != self.model:
            self.log(f"  Pass 1/6 — Element Extraction ({self.extraction_model})...")
        else:
        ...
        result = self._call_json(
            ...
            PASS1_SYSTEM, "pass1",
            model_override=self.extraction_model,  # Use bigger model here
        )
    ...
    def pass6_summary(self, case_no: str, p1: dict, p2: dict,
        ...
        t = time.time()
        model_desc = self.model
        if self.extraction_model != self.model:
            model_desc = f"{self.extraction_model} (extraction) + {self.model} (analysis)"
        result = self._call(
            PASS6_PROMPT.format(
                case_no=case_no, tumor_type=self.tumor_type, model=model_desc,
    ...
    def analyze(self, report_text: str, macroscopy_text: str = "",
        ...
        if self.extraction_model != self.model:
            self.log(f"  Hybrid mode: Pass 1={self.extraction_model}, Passes 2-6={self.model}")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # 3B model (fast, basic accuracy)
  python %(prog)s -p ollama -f report.txt

  # 8B model (slower, better accuracy)
  python %(prog)s -p ollama -m llama3.1:8b -f report.txt

  # HYBRID: 8B for extraction, 3B for analysis (best speed/quality balance)
  python %(prog)s -p ollama -m llama3.2:latest -e llama3.1:8b -f report.txt

  # Batch mode
  python %(prog)s -p ollama -m llama3.2:latest -e llama3.1:8b --batch data.xlsx -o results/
""", formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    parser.add_argument("--extraction-model", "-e", type=str,
                        help="Bigger model for Pass 1 extraction (e.g. llama3.1:8b). "
                             "Other passes use --model (faster, smaller).")
